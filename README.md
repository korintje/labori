...
Web UIは、QCMの日常測定で必要になる実験文脈をセッションと一緒に保存します。

- 測定名
- サンプル名
- 実験メモ
- 測定中または履歴確認中のイベントマーカー
- baseline設定
- `frequency` / `Δf` 表示切り替え
- サンプル数、最新値、平均、標準偏差、driftの簡易表示

単一チャンネル画面では、読み込まれているデータまたはライブデータをbaselineとして設定し、`Δf = f - f0` 表示に切り替えられます。baseline設定はイベントにも記録されます。

イベントマーカーは、添加、洗浄、バッファ交換、baseline、メモなどを測定時刻付近の連番とともに保存します。CSV exportには、測定条件、メモ、イベント、baselineがある場合のΔf列が含まれます。
...
`at_sequence` は省略できます。省略時は `-1` として保存されます。
